		...
		try {
			...
			for (const [name, value] of formData.entries()) {
				if (
					...
					// Stream file to disk first (memory efficient)
					...
					if (uploadToAppwrite) {
						try {
							// Upload from disk file instead of memory-loaded file
							const appwriteResult = await uploadToAppwriteBucketFromPath(
								filePath,
								file.name,
								file.type,
								file.size,
