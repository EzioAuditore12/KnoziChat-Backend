...
import type { RespondToRequest } from "@/routes/chats/private-chat.route";
import { emitEvent } from "@/utils/socket-io";
import { eq } from "drizzle-orm";

export const respondToRequest: AuthenticatedAppRouteHandler<
	RespondToRequest
> = async (c) => {
	...
	if (action === "accept") {
		...
		emitEvent(
			...
			"Friend request accepted! New chat created.",
		);
		emitEvent(
			c,
			ALERT,
			[friendRequest.senderId],
			`${friendRequest.senderName} accepted your friend request!`,
		);
