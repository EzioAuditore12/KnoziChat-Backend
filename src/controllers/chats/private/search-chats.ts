...
import type { SearchPrivateChats } from "@/routes/chats/private-chat.route";
import { and, eq, ilike, inArray, or } from "drizzle-orm";

export const searchChats: AuthenticatedAppRouteHandler<
	SearchPrivateChats
> = async (c) => {
		...
		.where(
			and(
				...
				or(
					...
					and(
						eq(chatsTable.groupChat, true),
						ilike(chatsTable.name, `%${name}%`),
					),
	...
	const privateChatsWithMembers = await db
		...
		.where(
			and(
				...
				eq(chatsTable.groupChat, false),
				ilike(usersTable.firstName, `%${name}%`),
				// Exclude current user from the search
				// (we want to find chats where OTHER user's name matches)
			),
		);

	// Filter private chats to only include ones where the OTHER user matches the query
	const validPrivateChats = privateChatsWithMembers.filter(
	...
	const allMatchingChats = [
		...groupChatResults,
		...privateChatResults.filter(
			(privateChat) =>
				!groupChatResults.some((groupChat) => groupChat.id === privateChat.id),
		),
	];
