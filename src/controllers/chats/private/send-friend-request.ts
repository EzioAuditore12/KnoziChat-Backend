...
import type { SendFriendRequest } from "@/routes/chats/private-chat.route";
import { emitEvent } from "@/utils/socket-io";
import { and, eq, or } from "drizzle-orm";

export const sendFriendRequest: AuthenticatedAppRouteHandler<
	SendFriendRequest
> = async (c) => {
	...
	const existingRequest = await db
		...
		.where(
			or(
				and(
					eq(requestsTable.sender, senderId),
					eq(requestsTable.receiver, receiverId),
				),
				and(
					eq(requestsTable.sender, receiverId),
					eq(requestsTable.receiver, senderId),
				),
			),
		)
		.limit(1);
	...
	if (senderChatIds.length > 0) {
		const mutualChat = await db
			...
			.where(
				and(
					eq(chatMembersTable.userId, receiverId),
					// Check if receiver is in any of sender's chats
					or(
						...senderChatIds.map((chatId) =>
	...
	emitEvent(
		...
		ALERT,
		[receiverId],
		`You have a new friend request from ${receiver[0].name}`,
	);
