...
import { defaultFileNameGenerator, removeFileSync, streamFileToDisk, } from "../utils/file-operations.js";
import { createMiddleware } from "hono/factory";
export function honoMulter({ dest = "./public/temp", allowedTypes = [], maxSize = 5 * 1024 * 1024, fileNameConvertor = defaultFileNameGenerator, fieldNames = [], requireAllFields = false, uploadToAppwrite = false, }) {
    return createMiddleware(async (c, next) => {
        try {
            ...
            for (const [name, value] of formData.entries()) {
                if (typeof value === "object" &&
                    ...
                    if (fieldNames.length > 0 && !fieldNames.includes(name)) {
                        throw new Error(`File field '${name}' not allowed. Allowed fields: ${fieldNames.join(", ")}`);
                    }
                    ...
                    if (file.size > maxSize) {
                        throw new Error(`File size ${file.size} exceeds limit of ${maxSize} bytes`);
                    }
                    ...
                    if (uploadToAppwrite) {
                        try {
                            ...
                            const appwriteFile = new File([new Uint8Array(fileBuffer)], file.name, { type: file.type });
                            const appwriteResult = await uploadToAppwriteBucket(appwriteFile);
                            if (appwriteResult) {
                                uploadedFile.appwrite = appwriteResult;
                                console.log(`File uploaded to Appwrite: ${appwriteResult.fileId}`);
                                // Remove local file after successful Appwrite upload
                                removeFileSync(filePath);
                                console.log(`Local file removed: ${filePath}`);
                            }
                            else {
                                console.warn(`Failed to upload ${file.name} to Appwrite, keeping local file`);
                            }
                        }
                        catch (appwriteError) {
                            console.error(`Appwrite upload failed for ${file.name}:`, appwriteError);
            ...
            if (requireAllFields && fieldNames.length > 0) {
                const missingFields = fieldNames.filter((fieldName) => !foundFieldNames.has(fieldName));
                if (missingFields.length > 0) {
                    throw new Error(`Missing required file fields: ${missingFields.join(", ")}`);
                }
            }
            // Check if at least one file was uploaded (when fieldNames are specified)
            if (fieldNames.length > 0 && foundFieldNames.size === 0) {
                throw new Error(`No files uploaded. Expected fields: ${fieldNames.join(", ")}`);
            }
