...
export const deleteGroupChat = async (c) => {
    ...
    if (!chat.groupChat)
        return c.json({ message: "Given chat id is not a group chat" }, HTTPStatusCode.BAD_REQUEST);
    const [member] = await db
        .select()
        .from(chatMembersTable)
        .where(and(eq(chatMembersTable.chatId, chatId), eq(chatMembersTable.userId, userId)));
    if (!member) {
        return c.json({ message: "You are not a member of this chat" }, HTTPStatusCode.BAD_REQUEST);
    }
    if (chat.creatorId !== userId)
        return c.json({ message: "You are not allowed to delete this group" }, HTTPStatusCode.UNAUTHORIZED);
