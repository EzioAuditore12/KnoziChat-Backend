...
export const addGroupMembers = async (c) => {
    ...
    if (existingUsers.length !== members.length) {
        return c.json({ message: "One or more members are not registered users" }, HTTPStatusCode.BAD_REQUEST);
    }
    // Find already existing members in the chat
    const existingMembers = await db
        .select({ userId: chatMembersTable.userId })
        .from(chatMembersTable)
        .where(and(eq(chatMembersTable.chatId, chatId), inArray(chatMembersTable.userId, members)));
    ...
    if (newMembersToAdd.length === 0) {
        return c.json({ message: "All provided members are already in the group" }, HTTPStatusCode.BAD_REQUEST);
    }
    // Add new members
    await db
        .insert(chatMembersTable)
        .values(newMembersToAdd.map((userId) => ({ chatId, userId })));
    emitEvent(c, ALERT, newMembersToAdd, `All Users have been added to ${chat.name}`);
    return c.json({ message: "Members added successfully", addedMembers: newMembersToAdd }, HTTPStatusCode.CREATED);
};
